...
`endif
	...
	input wire i_axi_ctrl; // My Property
...
`endif
	...
	wire	[11:0]	pf_return_v, pf_return_p;
...
`ifdef	OPT_MMU
	...
	zipmmu	#(	.LGTBL(LGTLBSZ),
		...
		themmu(i_clk, cpu_reset,
			...
			cpu_gbl_cyc, cpu_gbl_stb, cpu_we, 1'b0, cpu_addr,
				cpu_data, cpu_sel, 1'b0,
		...
		/*
		module zipmmu(i_clk, i_reset, 
			// Slave interface
			i_wbs_cyc_stb, 
			i_wbs_we, i_wbs_addr, i_wbs_data, 
			o_wbs_stall, o_wbs_ack, o_wbs_data,
			// CPU global bus master lines
			i_wbm_cyc, i_wbm_stb, i_wbm_we, i_wbm_exe, i_wbm_addr, 
			i_wbm_data, i_wbm_sel, i_gie,
			// MMU bus master outgoing lines
			o_cyc, o_stb, o_we, o_addr, o_data, o_sel,
			// .... and the return from the slave(s)
			i_stall, i_ack, i_err, i_data,
			// CPU gobal bus master return lines
			o_rtn_stall, o_rtn_ack, o_rtn_err, o_rtn_miss, o_rtn_data,
			pf_return_stb, pf_return_we, pf_return_p, pf_return_v,
				pf_return_cachable);
		*/
...
`endif
	...
	assign	cpu_stall = mmu_cpu_stall;
