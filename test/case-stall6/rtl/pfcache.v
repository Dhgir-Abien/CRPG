...
`ifdef F_MAX_STALL
	...
	reg	is_wb_active;
	initial is_wb_active = 0;
	always @(posedge i_clk) begin
		if(i_reset)
			is_wb_active <= 0;
		else if((o_wb_stb) && (o_wb_cyc))
			is_wb_active <= 1;
		else
			is_wb_active <= is_wb_active;
	end
	
	localparam	MAX_STALL = 24;
	reg	[4:0] stall_count;
	initial stall_count = 0;
	always @(posedge i_clk) begin
		if ((!i_reset)  && (is_wb_active) && i_wb_stall)
			stall_count <= stall_count + 1'b1;
		else
			stall_count <= 0;
	end
	always @(*)
		assert(stall_count < MAX_STALL);
`else
...
`endif
	...
	fwb_master #(.AW(AW), .DW(BUSW), .F_LGDEPTH(F_LGDEPTH),
			.F_MAX_STALL(MAX_STALL), .F_MAX_ACK_DELAY(3),
