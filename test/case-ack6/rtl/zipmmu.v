...
`ifdef F_CYC_STABLE
	// CYC Stable Condition
	reg pass_i_wbm_cyc;
	initial	pass_i_wbm_cyc = 1'b0;
	always @(posedge i_clk)
	if ((!i_reset))
		pass_i_wbm_cyc <= i_wbm_cyc;
	else
		pass_i_wbm_cyc <= 0;

	reg pass_o_rtn_ack;
	initial	pass_o_rtn_ack = 1'b0;
	always @(posedge i_clk)
	if ((!i_reset))
		pass_o_rtn_ack <= o_rtn_ack;
	else
		pass_o_rtn_ack <= 0;

	always @(posedge i_clk)
		if(pass_i_wbm_cyc && !pass_o_rtn_ack)
			assert(i_wbm_cyc);
`endif
